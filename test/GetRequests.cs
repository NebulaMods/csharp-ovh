...
public class GetRequests
{
    ...
    public async Task GET_auth_time()
    {
        ...
        Assert.Equals(2, await c.GetTimeDelta());
    }
    ...
    public async Task GET_me_as_string(string call, string called, string sig)
    {
        ...
        Assert.Equals(Responses.Get.me_content, result);
	    ...
	    Assert.Equals(called, uri.AbsoluteUri);
        ...
        Assert.Multiple(() => {
            ...
            Assert.Equals(sig, headers.GetValues(Client.OVH_SIGNATURE_HEADER).First());
        });
    }
    ...
    public async Task GET_me_as_T()
    {
        ...
        Assert.Equals("Noname", result.name);
        Assert.Equals("none-ovh", result.nichandle);
        Assert.Equals("EUR", result.currency.code);
        Assert.Equals("€", result.currency.symbol);
    }
    ...
    public async Task GET_with_filter_generates_correct_signature()
    {
        ...
        var requestMessage = meCall.GetArgument<HttpRequestMessage>("request");
        var headers = requestMessage.Headers;
        Assert.Equals("$1$098b93d342b6db4848ec448063be2b6884e94723", headers.GetValues(Client.OVH_SIGNATURE_HEADER).First());
    }
}
